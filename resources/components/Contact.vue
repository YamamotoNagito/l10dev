  ...
  if (!v$.value.$invalid) {
    ...
    try {
      ...
      if (error.response) {
        ...
        alert(
          "エラー：何らかの原因によりお問い合わせ内容を送信できませんでした. 復旧までの間, お問い合わせをする場合は tarouhirodai@gmail.com までご連絡ください. "
        );
      }
    }
  }
...
<template>
  <v-container>
    ...
    <v-text-field
      v-model="name"
      :error-messages="
        v$.name.$error ? ['1字以上32字以下の, 氏名を入力してください. '] : []
      "
      label="氏名"
      placeholder="広島 かえで"
      hide-details="auto"
    ...
    <v-select
      ...
      :error-messages="
        v$.category.$error
          ? [
              'お問い合わせの種類を選択してください. 当てはまるものが無ければ「その他」を選択してください. ',
            ]
          : []
      "
