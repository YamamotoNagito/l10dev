...
class LecturesController extends Controller
{
    ...
    public function searchByConditions(Request $request)
    {
        ...
        $lectureIds = DB::table('lectureDetailTimes')
                    ...
                    ->leftJoin('reviews', 'lectures.lectureId', '=', 'reviews.lectureId')
                        ...
                        foreach ($selectedConditions as $conditionKey => $conditionValue) {
                            switch ($conditionKey) {
                                ...
                                case 'totalEvaluationMin':
                                    Log::Debug(intval($conditionValue));
                                    $query->having(DB::raw('(AVG(reviews.skillLevel) + AVG(reviews.interestLevel) + AVG(reviews.creditLevel)) / 3'), '>=', intval($conditionValue));
                                    break;
                                ...
                                case 'creditLevelMin':
                                    $query->groupBy('lectures.lectureId')  
                                          ->having(DB::raw('AVG(reviews.creditLevel)'), '>=', intval($conditionValue));
                                    break;
                                    
                                case 'creditLevelMax':
                                    $query->having(DB::raw('AVG(reviews.creditLevel)'), '<=', intval($conditionValue));
                                    break;
                                          
                                case 'interestLevelMin':
                                    $query->having(DB::raw('AVG(reviews.interestLevel)'), '>=', intval($conditionValue));
                                    break;
                                ...
                                case 'skillLevelMin':
                                    $query->having(DB::raw('AVG(reviews.skillLevel)'), '>=', intval($conditionValue));
                                    break;
